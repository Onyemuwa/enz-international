...
export function legalPage(lang, type) {
  ...
  const content = isPrivacy
    ? `<p>ENZ INTERNATIONAL ("we", "us") collects only what you type into a form on this site — your name, email address, phone number, company and the details of your enquiry — and uses it only to reply to you.</p>
       ...
       <h2 >How your submission reaches us</h2><p>When you submit a form, what you typed is sent to our own server and saved in a database there, and a copy is emailed to us so we can reply. The server is hosted by Railway, and the notification email is delivered through an email-delivery service (Resend). If our server cannot be reached, the form may instead pass your submission to a backup third-party form-delivery service, which relays it to our inbox and may hold it briefly so a message is not lost in transit. If that also fails, the form opens your own email application with the enquiry pre-written, and nothing is sent until you press send.</p>
       <h2 >How long we keep it</h2><p>Enquiries, career applications (including any CV you attach) and newsletter addresses stay in our database, and enquiries in our email, for as long as we may reasonably need them to deal with your request and any follow-up. Ask us to delete yours and we will remove it from both.</p>
